...
function  detectedOOIs = partD(LaserData, x, landmarks)
x(1) = -x(1);
x(3) = pi - x(3);
myPosition = x;
...
Handles.H3 = quiver(myPosition(1),myPosition(2),cos(myPosition(3)),sin(myPosition(3)),'r');%plot position
...
if ~isempty(landmarks)
    ...
    for i=1:length(detectedOOIs(1,:))
        ...
        if index > 0
            set(handle(index),'position',detectedOOIs(:,i));
        end
    end
end
return;
