...
% by J. Guivant for MRTN4010's students, Session S2.2015.
...
% .....................................................
% Here I define the magnitude of the "noises", that I assume are present, 
...
% Speed meter's error = 0.3m/sec; actually, a low quality speed sensor!
...
% perdiod of time of Dt (50ms) we introduce uncertainty of 0.01m standard deviation on X, Pu = diag([(0.01)^2, (1*pi/180)^2]);
...
%my modifications, keep original q matrix as initial, set up Pu diagonal
Qi = diag( [ (0.01)^2 ,(0.01)^2 , (1*pi/180)^2]) ;

Pu = diag([(stdDevSpeed)^2, (stdDevGyro)^2]);

time=0 ;
% init simulator of process (the "real system").
InitSimulation(stdDevSpeed,stdDevGyro,stdDevBear, sdev_rangeMeasurement,DtObservations);
...
for i=1:Li,     % loop
    ...
    Ju = [ [cos(Xe(3))*Dt,0] ; [sin(Xe(3))*Dt,0] ; [0,Dt] ];
    ...
    [nDetectedLandmarks,MasuredRanges,IDs,MeasuredBearings]=GetObservationMeasurements();
    ...
    if nDetectedLandmarks>0,     % any laser data and detected landmarks?
        ...
        % ----------3)----- Kalmal Filter update (observations)
        ...
        % We perform an EKF update for each of the observations
        for u=1:nDetectedLandmarks,
            ...
            H = [  eDY/eD2 , -eDX/eD2, -1 ] ;   % Jacobian of h(X); size 1x3
            ...
            zRange  = MasuredRanges(u) - ExpectedRange ;      
            zBearing = MeasuredBearings(u) - ExpectedBearing;
            zBearing = mod(zBearing + pi, 2*pi)-pi;

            
            % ------ covariance of the noise/uncetainty in the measurements
            RRange = sdev_rangeMeasurement*sdev_rangeMeasurement*4 ;
            RBearing = 4 * stdDevBear * stdDevBear;
            ...
            Xe = Xe+K*zBearing ;           % update the  expected value
...
function InitSimulation(stdDevSpeed,stdDevGyro, stdDevBear, sdev_rangeMeasurement,DtObservations)
    ...
    ContextSimulation.stdDevBear = stdDevBear;
...
function [nDetectedLandmarks,MasuredRanges,IDs, MeasuredBearings]=GetObservationMeasurements(map)
    ...
    % simulated platform.
        ...
        if (nDetectedLandmarks<1)       % no detected landmarks...
            MasuredRanges=[];   IDs=[];  MeasuredBearings=[];  return ; 
        end;
        ...
        % this is the noise:
        noiseInMeasurementsRange= ContextSimulation.sdev_rangeMeasurement*randn(size(RealRanges));
        noiseInMeasurementsBearings= ContextSimulation.stdDevBear*randn(size(RealBearings));

        % here I add it to the perfect ranges' measurements
        MasuredRanges = RealRanges +  noiseInMeasurementsRange ;
        MeasuredBearings = RealBearings + noiseInMeasurementsBearings;
...
%  Jose Guivant - For Projects 01/02 - AAS -Session 1.2016
