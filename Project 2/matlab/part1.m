...
fprintf('New samples: IMU1=[%d],LMS200a=[%d],SpeedMeter=[%d]\n',rI1.n,rLa.n,rS.n)
    ...
    GyroZ = rI1.data(6,1:rI1.n) + 0.0170;
    ...
    for i=2:rI1.n 
        ...
        J = [ [1,0,-dt*speed(speedIndex)*sin(x(3))  ]  ; [0,1,dt*speed(speedIndex)*cos(x(3))] ;    [ 0,0,1 ] ] ;
        Ju = [ [cos(x(3))*dt,0] ; [sin(x(3))*dt,0] ; [0,dt] ];
        ...
        if startFlag == 0
            [P, x] = EKFlocal(x, landmarks, OOIs, P, R);
            history(:,count) = x;
            count = count + 1;
        end
